...
sub to_type
{
  ...
  return 'generic function/package/procedure' if ($char eq 'G');
  return 'integer'                            if ($char eq 'I');
...
sub create_new_reference
{
    ...
    $refline += $offset;
    if ($type eq "b" && $symbols {$currentref} eq 'f')
    {
...
sub parse_ali
{
  ...
  foreach (@ali)
  {
	...
	my ($file, $offset) = ($1, (defined $3) ? 2 - $3 : 0);
    ...
    elsif (/^(\d+)(.)(\d+)[ *]([\w\d-.]|".")+\s+(.*)/ && defined $currentfile)
    {
...
sub output_file
{
  ...
  local ($alifilename) = $filename_param;
  $alifilename =~ s/\.ad[sb]$/.ali/;
  ...
  &parse_ali (&get_real_file_name ($alifilename));
