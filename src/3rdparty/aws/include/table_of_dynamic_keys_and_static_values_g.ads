...
package Table_Of_Dynamic_Keys_And_Static_Values_G is
   ...
   --  entries of type (KEY_TYPE, VALUE_TYPE), where KEY_TYPE and VALUE_TYPE
   --  are specified by generic parameters. Such a couple will also be called
   --  an item.
   ...
   --    The following consistency condition must be fullfilled by the
   --  relational operations LESS and EQUALS:
   ...
   --  type nor an access type. When an actual generic access type is
   --  associated with a generic static type, objects would be shared, i.e.
   --  only the access value would be stored, without copying the accessed
   --  object.
   ...
   --  structure share the item with the structure and do not return a copy of
   --  it. This may have consequences if the type of the item, (or some
   --  component of it) is an access type. For instance, when accessing an
   --  item by a function call, this item must not be destroyed or modified
   --  during the query.
   --+
   --  PRIMITIVES:
   --    CONSTRUCTORS:
   ...
   --  using pointers. The items are sorted in the table by increasing keys
   --  values in conformance to inorder.
   ...
   function Less_Or_Equal_Key (Table : in Table_Type; Key : in Key_Type)
     return Key_Type;
...
private
   ...
   type Table_Type is
      record
         ...
         --  Used to connect alternatively the predecessor or successor when
         --  deleting a node. Is provided for optimization.
      end record;
