...
package body Table_Of_Static_Keys_And_Static_Values_G is
...
--/ LOCAL SUBPROGRAM:
   function Search_A_Key (Root : in Link_Type; Key : in Key_Type)
      return Link_Type is
...
--/ QUERIES:
   ...
   function Is_Present (Table : in Table_Type; Key : in Key_Type)
      return Boolean is
   begin -- IS_PRESENT
...
--/ LOCAL SUBPROGRAM:
   function Search_Less (Root : in Link_Type;
                         Key : in Key_Type) return Link_Type is
   --  Result points to the cell with key value less than KEY;
   --  when search fails, null value is returned.
      Ptr : Link_Type := Root;
      Best : Link_Type;
   begin
...
--/ LOCAL SUBPROGRAM:
   ...
   function Greater_Key (Table : in Table_Type; Key : in Key_Type)
      return Key_Type is
      Ptr : constant Link_Type := Search_Greater (Table.Root, Key);
   begin
      if Ptr = null then
...
--/ SET OPERATIONS:
   ...
   package body Set_Operations_G is
      ...
      function "=" (Left, Right : in Table_Type) return Boolean is
         --  Set equality; the LEFT and RIGHT tables contain
         --  entries with same values
      ...
      begin -- "<"
         ...
         for Left_Index in 1 .. Left.Count loop
            ...
            if not Found then
            --  item associated with LEFT_INDEX has not been found
            --  in RIGHT table.
               return False;
            end if;
         end loop;
...
--/ HEAP MANAGEMENT:
   ...
   begin
      ...
      Table_Of_Static_Keys_And_Static_Values_G.Max_Free_List_Size :=
                                                         Max_Free_List_Size;
...
end Table_Of_Static_Keys_And_Static_Values_G;
=======
--------------------------------- COPYRIGHT ----------------------------------
--  (C) 1987 Swiss Federal Institute of Technology (EPFL).
--      Represented by A. Strohmeier EPFL-DI-LGL CH-1015 Lausanne Switzerland.
