...
--  $Id: zlib.ads,v 1.2 2003/10/09 16:27:02 Jano Exp $
...
package ZLib is
   ...
   type Header_Type is (None, Auto, GZip);
   ...
   --  RLE stategy could be used only in version 1.2.0.1 and later.
   ...
   --  When Header is Auto, then default zlib header would be provided.
   ...
   --  Header type None available only for versions 1.2.0 and later.
   --  Header type None mean that no header is expecting in the input stream.
   --  Auto header type mean that header is deault zlib header.
   --  Versions 1.2.0 and later could detect gzip/zlib headers itself if the
   --  Header parameter is not None.
