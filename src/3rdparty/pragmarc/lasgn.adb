...
procedure Lasgn is
   ...
   procedure Put
      (Item : in out Int_Wrapping.Wrapped_Scalar; Context : in out Integer; Pos : in Int_List.Position; Continue : out Boolean)
   is
      -- null;
   begin -- Put
      Put_Line (Item => Integer'Image (Item.Value) );
      Continue := True;
   end Put;
...
begin -- Lasgn
   Fill : for I in 1 .. 10 loop
      if I rem 2 = 0 then
         Int_List.Insert (Into => First, Item => (Value => I), Before => Off, New_Pos => New_Pos);
      else
         Int_List.Append (Into => First, Item => (Value => I), After => Off, New_Pos => New_Pos);
      end if;
   end loop Fill;
