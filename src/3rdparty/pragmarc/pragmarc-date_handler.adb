...
package body PragmARC.Date_Handler is
   ...
   function Image (Value : Natural; Width : Natural := 0; Zero_Fill : Boolean := True) return String is
      Default : String := Integer'Image (Value);
      
      Fill   : Character := ' ';
      Result : String (1 .. Width);
   begin -- Image
      if Default'Length > Width then
         return Default (Default'First + 1 .. Default'Last);
      end if;
      
      if Zero_Fill then
         Fill := '0';
         Default (Default'First) := Fill;
      end if;
      
      Move (Target => Result, Source => Default, Justify => Right, Pad => Fill);
