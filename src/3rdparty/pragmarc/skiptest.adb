...
procedure Skiptest is
   ...
   procedure Put (Item : in Int; Count : in Natural) is
      Row    : constant Positive := (Count - 1) rem Num_Lines + 1;
      Column : constant Positive := Num_Columns * ( (Count - 1) / Num_Lines) + 1;
   begin -- Put
      Text_IO.Put (ATC.Position (Row, Column) & Integer'Image (Item.Value) );
   end Put;

   Count : Natural := 0;

   procedure Print_One (Item : in out Int; Count : in out Natural; Continue : out Boolean) is
      -- null;
   begin -- Print_One
      Continue := True;
      Count := Count + 1;
...
begin -- Skiptest
   ...
   Fill_1 : for I in Number'range loop
      Data := (Value => Number (I) );
      Put (Item => Data, Count => I);
      Skip.Insert (List => List, Item => Data);
   end loop Fill_1;
   ...
   Check : for I in Stored_Integer loop
      Search := Skip.Search (List, (Value => I) );

      if Search.Found then
         Count := Count + 1;
         Put (Item => (Value => I), Count => Count);
      end if;
   end loop Check;
