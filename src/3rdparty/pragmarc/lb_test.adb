...
begin -- LB_Test
   Add_10 : for I in 1 .. 10 loop
      if I rem 2 /= 0 then
         Int_List.Insert (Into => List, Item => (Value => I), Before => Int_List.First (List), New_Pos => Pos);
      else
         Int_List.Append (Into => List, Item => (Value => I), After => Int_List.Last (List), New_Pos => Pos);
      end if;
   end loop Add_10;
